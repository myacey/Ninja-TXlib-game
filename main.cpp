...
int main()
{
    ...
    while ((yasuo.alive == true or time1 - earthquake_start < 1000) and !yasuo_won and !ninja.won and !nadoelo)
    {
        ...
        if (
            ...
            ninja.y       < tovary[nomer_vybora_yasuo].y + 100
           )
            {
                yasuo_won = true;
            }
        ...
        if (GetAsyncKeyState(VK_ESCAPE) and yasuo.alive == true)
        {
            ...
            //Настройки
            RECT rectSettings = {
                (txGetExtentX() - WIDTH_MENU) / 2 + 118,
                (txGetExtentY() - HEIGHT_MENU) / 2 + 118,
                (txGetExtentX() - WIDTH_MENU) / 2 + 500,
                (txGetExtentY() - HEIGHT_MENU) / 2 + 155};
            //Возвращение в игру
            RECT rectReturn = {
            ...
            RECT rectTest = {
                (txGetExtentX() - WIDTH_MENU) / 2 + 120,
                (txGetExtentY() - HEIGHT_MENU) / 2 + 300,
                (txGetExtentX() - WIDTH_MENU) / 2 + 500,
                (txGetExtentY() - HEIGHT_MENU) / 2 + 340};
            ...
            while (!returnToGame)
            {
                ...
                if (In (txMousePos(), rectSettings) && txMouseButtons() == 1)
                {
                    HDC setings = txLoadImage("Меню\\main настройки.bmp");

                    txBitBlt(txDC(), 0, 0, 1309,550, Fon);
                    txTransparentBlt(txDC(),
                        (txGetExtentX() - WIDTH_MENU) / 2,
                        (txGetExtentY() - HEIGHT_MENU) / 2,
                        WIDTH_MENU,HEIGHT_MENU, setings, 0, 0, TX_WHITE);
                    txSleep(5000);
                    returnToGame = true;
                }


                if(In (txMousePos(), rectTest) && txMouseButtons() == 1)
                {
                    while(In (txMousePos(), rectTest) && txMouseButtons() == 1)
                    {
                        txSleep(10);
                    }

                    ShellExecute(NULL,NULL,"Test.exe",NULL ,NULL,SW_SHOWMINIMIZED);
